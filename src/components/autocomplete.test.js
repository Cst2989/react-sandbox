...
describe('<Autocomplete /> with mocked data', () => {
    ...
    it('should set the active data value prop on Focus', () => {
        ...
        expect(container.find('input[type="text"]').prop('data-active')).toEqual(true);
    });

    it('should hide the active data value prop on Blur', () => {
        container.find('input[type="text"]').simulate('focus', {});
        container.find('input[type="text"]').simulate('blur', {});
        expect(container.find('input[type="text"]').prop('data-active')).toEqual(false);
    });

    it('should increase the activeItem on arrowDown', () => {
        container.find('input[type="text"]').simulate('keydown', {key: 'ArrowDown'})
        expect(container.find('input[type="text"]').prop('data-active-item')).toEqual(1);
    });

    it('should decrease the activeItem on arrowUp', () => {
        container.find('input[type="text"]').simulate('keydown', {key: 'ArrowDown'})
        container.find('input[type="text"]').simulate('keydown', {key: 'ArrowUp'})
    ...
    it('should render a list based on the mocked data', () => {
        ...
        const list = testInstance.findAll(
            (el) => el.type === 'li'
            );
