...
const prepareManagerData = dataObject => {
  return dataObject.data.map(employee => {
    return {
      ...
      initials:
        employee.attributes.firstName[0] + employee.attributes.lastName[0],
      email: dataObject.included.find(
        account => account.id === employee.relationships.account.data.id
      ).attributes.email
    };
  });
};
...
const HomeForm = () => {
  ...
  return (
    <Fragment>
      <h3 className="heading-3 l-center">
        {" "}
